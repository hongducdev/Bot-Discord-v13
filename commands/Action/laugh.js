...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        let body = await fetch (`http://api.nekos.fun:8080/api/laugh`)
        const data = await body.json()
        if (!data) return message.channel.send('Lỗi vui lòng thử lại sau');
            let laughEmbed = new MessageEmbed()
                .setColor('GREEN')
                .setAuthor(`Laugh`, message.guild.iconURL)
                .setImage(data.image)
                .setTimestamp()
                .setFooter(`Bot: Duwc510_ by hongduccodedao`, client.user.displayAvatarURL)

        msg.edit({ content:'\u200b',embeds: [laughEmbed] });
    }
}